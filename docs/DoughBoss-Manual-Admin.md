...
- **$5 coupon rule UID** / **$10 coupon rule UID** — the POSPal coupon-rule identifiers for the two voucher values DoughBoss currently mirrors. Leaving both blank keeps the "grant" side fully dormant even with POSPal otherwise enabled (see §6, Troubleshooting).
- **Verify coupon setup** — a button that runs a read-only check against the selected store (dropdown just above it) confirming the connection and that the UIDs correspond to real coupon rules. **Save your changes first** — it tests the stored settings, not whatever is currently typed in the form.
- **Test grant** — enter a *throwaway* phone number and a $5/$10 value, then **Send test coupon** to actually write a test member + grant a coupon in POSPal and see the raw API response. Use **Revoke test** afterwards to undo it. **Probe methods** is a diagnostic tool that tries a list of candidate POSPal endpoint names against your live account — a developer/support tool, not something to click routinely.
