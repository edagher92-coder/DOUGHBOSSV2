...
- **The standalone Staff Console app** (the online add-to-home-screen app used for Scan/Vouchers/Order-Board outside of wp-admin) authenticates via a WordPress Application Password. Console 2.17.0 and later keeps that password in memory only: reloading or closing the page signs the operator out. The site address and username may be remembered, but the password is not written to browser storage. Revoke the device's Application Password immediately if a signed-in tablet is lost or replaced. A same-origin, short-lived device-session design remains the recommended production architecture.
