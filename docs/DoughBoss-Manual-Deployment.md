...
3. **Confirm POSPal is in the intended state**, if used: Settings → POSPal POS status line ("POSPal is configured and enabled" vs. "not connected yet"), and specifically whether the coupon-grant leg is on (requires the master toggle **and** at least one $5/$10 coupon-rule UID filled in for at least one store) — this is easy to have half-configured without noticing, per the troubleshooting guidance in `docs/DoughBoss-Manual-Admin.md` §7.
