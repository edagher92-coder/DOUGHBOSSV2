...
When the optional Order Board key is enabled, the KDS order feed and its accept/ack/status actions require that key in addition to the WordPress staff credential. Open the console with the same one-time key appended as `?key=...`; the console keeps it in memory and sends it as a protected request header. The key is not written to local storage. Regenerating the key invalidates both the wp-admin board bookmark and any console URL using the previous value. This protects order data/actions, but it still does not hide the static login shell itself.
