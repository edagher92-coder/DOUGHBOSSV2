...
check(shortcodes.includes('doughboss-feast-real-v1.jpg') && fs.existsSync(path.join(root, 'public/images', 'doughboss-feast-real-v1.jpg')), 'hero uses the approved real Dough Boss merchant photograph');
check(hero.includes("setProperty('--db-mh-photo-y'") && hero.includes("window.addEventListener('scroll'"), 'real hero photograph follows scroll with restrained reversible parallax');
check(hero.includes("window.addEventListener('resize'"), 'hero recalculates its photographic scene responsively');
check(!shortcodes.includes('db-mh-ingredient') && !shortcodes.includes('db-mh-central'), 'generated floating-food layers are absent from production markup');
