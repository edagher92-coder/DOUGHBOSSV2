...
preorder_ok( false !== strpos( $files['admin'], "'screenMode' => \$screen_mode" ) && false !== strpos( $files['board'], "['make', 'pass', 'catering']") && false !== strpos( $files['board'], "'/admin/catering-board'") && false !== strpos( $files['board'], 'renderCatering' ), 'KDS supports dedicated MAKE, PASS and catering touch-screen modes without changing its access boundary' );
preorder_ok( false !== strpos( $files['board'], "return o.order_type === 'dine_in' ? 'Served' : 'Collected';" ) && false !== strpos( $files['board'], "setStatus(o, st)" ), 'PASS screen collection action uses the existing version-checked status transition' );
