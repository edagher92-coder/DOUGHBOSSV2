	...
	function renderTableOccupancy() {
		...
		panel.appendChild(el('div', { class: 'db-table-occupancy-head' }, [
			el('div', {}, [
				...
				el('h2', { text: tablesLoaded ? reserved + ' reserved · ' + available + ' available' : 'Loading tables…' })
			]),
			el('p', { text: 'QR orders reserve a table for 15 minutes. A later order from that table renews the hold.' })
		]));
		if (!tablesLoaded) {
			panel.appendChild(el('p', { class: 'db-table-occupancy-empty', text: 'Loading the current table state…' }));
			return panel;
		}
		if (!tables.length) {
