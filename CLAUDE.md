...
- **Current code release:** plugin `2.33.0` / DB `1.18.0`. This retains the 2.32 payment and operations contracts, adds protected `/kitchen/` and `/management/` workspaces, rebuilds the homepage around distinct high-resolution food imagery and reliable blow-out/rebuild motion, supplies a distinct high-resolution photo for all 34 canonical menu products, tunes the 1920x1080 MAKE station and compact catering station, and keeps the Sujuk Special in-place product migration. Do not infer that production WordPress is running it until the live site responds and the installed version is checked.
...
Current consolidated release contract: plugin `2.33.0`, database schema `1.18.0`.
