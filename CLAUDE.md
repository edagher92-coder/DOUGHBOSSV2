...
- **Current code release:** plugin `2.34.0` / DB `1.18.0`. This retains the 2.32 payment and operations contracts, protected `/kitchen/`, hidden `/catering-kitchen/` and `/management/` workspaces, the Sujuk Special migration, Stripe voucher exclusivity and one-time student allocation/email delivery. The 2.34 visual release removes the generated floating-food hero and repeated/lookalike product photography: the homepage and supporting pages use approved real Dough Boss merchant photography with restrained accessible parallax, exact verified products map to their real photographs, and products without an exact approved photograph use a branded no-photo state. Do not infer that production WordPress is running it until the live site responds and the installed version is checked.
...
Current consolidated release contract: plugin `2.34.0`, database schema `1.18.0`.
