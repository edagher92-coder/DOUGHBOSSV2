...
final class DoughBoss_Deploy_2360_PclZip {
	const RESULT_OPTION     = 'doughboss_deploy_2360_pclzip_result';
	const JOURNAL_OPTION    = 'doughboss_deploy_2360_pclzip_journal';
	const DB_LOCK_OPTION    = 'doughboss_deploy_2360_pclzip_db_lock';
	const LOCK_FILE         = '.doughboss-deploy-2360-pclzip.lock';
	...
	public static function register_page() {
		add_plugins_page( 'DoughBoss verified deployment', 'DoughBoss 2.36 PclZip deployer', 'update_plugins', 'doughboss-deploy-2360-pclzip', array( __CLASS__, 'render_page' ) );
	}

	public static function register_routes() {
		register_rest_route( 'doughboss-deploy-2360-pclzip/v1', '/preflight', array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( __CLASS__, 'rest_preflight' ), 'permission_callback' => array( __CLASS__, 'allowed' ) ) );
	}
	...
	public static function render_page() {
		...
		if ( ! empty( $state['ready'] ) ) { echo '<form method="post">'; wp_nonce_field( 'doughboss_deploy_2360_pclzip' ); submit_button( 'Run verified deployment' ); echo '</form>'; }
		echo '</div>';
	}
...
DoughBoss_Deploy_2360_PclZip::init();
