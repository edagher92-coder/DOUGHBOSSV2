...
final class DoughBoss_Staff_Experience {
	...
	public function redirect_after_login( $redirect_to, $requested, $user ) {
		...
		// Honour the clock destination generated by wp_login_url(), but never an
		// arbitrary external redirect supplied by a request parameter.
		if ( $requested && false !== strpos( (string) $requested, '/staff-clock/' ) ) {
			$clock_url = home_url( '/staff-clock/' );
			$clock_parts = wp_parse_url( $clock_url );
			$request_parts = wp_parse_url( $requested );
			if ( is_array( $clock_parts ) && is_array( $request_parts ) && isset( $clock_parts['host'], $request_parts['host'] ) && strtolower( $clock_parts['host'] ) === strtolower( $request_parts['host'] ) ) {
				return $clock_url;
			}
		}
