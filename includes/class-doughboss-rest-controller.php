...
class DoughBoss_REST_Controller {
	...
	public function register_routes() {
		...
		// PII-free table service strip for the kitchen screens. QR bearer tokens,
		// table sessions and customer details are intentionally never exposed here.
		register_rest_route(
			$ns,
			'/admin/table-occupancy',
			array(
				'methods'             => WP_REST_Server::READABLE,
				'callback'            => array( $this, 'admin_table_occupancy' ),
				'permission_callback' => array( $this, 'verify_board_access' ),
				'args'                => array(
					'location_id' => array(
						'default'           => 0,
		...
		register_rest_route(
			$ns,
			'/admin/table/(?P<id>\d+)/reservation',
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( $this, 'admin_table_reservation' ),
				'permission_callback' => array( $this, 'verify_board_mutation' ),
				'args'                => array(
					'action' => array(
						'required'          => true,
						'sanitize_callback' => 'sanitize_key',
	...
	/**
	 * State-changing table controls need the normal KDS capability/key gate and
	 * the same-origin REST nonce rendered into the protected kitchen portal.
	 *
	 * @param WP_REST_Request $request Request.
	 * @return bool|WP_Error
	 */
	public function verify_board_mutation( WP_REST_Request $request ) {
		$allowed = $this->verify_board_access( $request );
		if ( is_wp_error( $allowed ) ) {
			return $allowed;
		}
	...
	/**
	 * GET /admin/table-occupancy — PII-free table labels and short holds.
	 *
	 * @param WP_REST_Request $request Request.
	 * @return WP_REST_Response|WP_Error
	 */
	public function admin_table_occupancy( WP_REST_Request $request ) {
		if ( ! DoughBoss_Activator::table_occupancy_storage_ready() ) {
			return new WP_Error( 'doughboss_table_storage_unavailable', __( 'Table service is temporarily unavailable while its storage is upgraded.', 'doughboss' ), array( 'status' => 503 ) );
		}
		$location_id = DoughBoss_Staff_Scope::effective_location_id( $request->get_param( 'location_id' ) );
		if ( is_wp_error( $location_id ) ) {
			return $location_id;
		}
		$tables = DoughBoss_Table_Occupancy::list_tables( $location_id );
		$reserved = count( array_filter( $tables, static function ( $table ) { return 'reserved' === $table['status']; } ) );
		$response = rest_ensure_response(
			array(
				'data' => $tables,
				'summary' => array(
					'total'     => count( $tables ),
					'reserved'  => $reserved,
					'available' => count( array_filter( $tables, static function ( $table ) { return 'available' === $table['status']; } ) ),
				),
				'server_time' => current_time( 'mysql', true ),
			)
		);
	...
	/**
	 * POST /admin/table/{id}/reservation — staff reserve or release.
	 *
	 * @param WP_REST_Request $request Request.
	 * @return WP_REST_Response|WP_Error
	 */
	public function admin_table_reservation( WP_REST_Request $request ) {
		if ( ! DoughBoss_Activator::table_occupancy_storage_ready() ) {
			return new WP_Error( 'doughboss_table_storage_unavailable', __( 'Table service is temporarily unavailable while its storage is upgraded.', 'doughboss' ), array( 'status' => 503 ) );
		}
		$table = DoughBoss_Table_Occupancy::get_table( absint( $request->get_param( 'id' ) ) );
		if ( is_wp_error( $table ) ) {
			return $table;
		}
		$location_id = DoughBoss_Staff_Scope::effective_location_id( (int) $table['location_id'] );
		if ( is_wp_error( $location_id ) ) {
			return $location_id;
		}
		...
		$args = array(
			...
			get_current_user_id(),
			sanitize_text_field( $request->get_param( 'event_key' ) ),
		);
		if ( 'reserve' === $action ) {
			$result = call_user_func_array( array( 'DoughBoss_Table_Occupancy', 'reserve_staff' ), $args );
		} elseif ( 'release' === $action ) {
			$result = call_user_func_array( array( 'DoughBoss_Table_Occupancy', 'release_staff' ), $args );
		} else {
			return new WP_Error( 'doughboss_table_reservation_action_invalid', __( 'Choose reserve or release.', 'doughboss' ), array( 'status' => 400 ) );
		}
		return is_wp_error( $result ) ? $result : rest_ensure_response( array( 'data' => $result ) );
	}
